...
SKILL_DOMAIN_MAP = {
    ...
    "python-lint-format-audit": "python-lint-format",
    "ts-lint-format-audit": "ts-lint-format",
...
def build_blocked_summary(skill_id: str, repo: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    if skill_id == "python-lint-format-audit":
        return simple_findings_blocked_summary(
            skill_id,
            repo,
            state,
            profile,
            summary_line="Shared toolchain bootstrap blocked the Python lint / format audit before modern Ruff evidence could be collected.",
            overall_verdict="scan-blocked",
        )
    if skill_id == "ts-lint-format-audit":
        return simple_findings_blocked_summary(
            skill_id,
            repo,
            state,
            profile,
            summary_line="Shared toolchain bootstrap blocked the TypeScript lint / format audit before modern Biome and typed-lint evidence could be collected.",
            overall_verdict="scan-blocked",
        )
    if skill_id == "openapi-jsonschema-governance-audit":
        return simple_findings_blocked_summary(
            skill_id,
            repo,
            state,
            profile,
            summary_line="Shared toolchain bootstrap blocked the schema-governance audit before artifact-chain evidence could be collected.",
            overall_verdict="scan-blocked",
        )
    if skill_id == "ts-frontend-regression-audit":
        return simple_findings_blocked_summary(
            skill_id,
            repo,
            state,
            profile,
            summary_line="Shared toolchain bootstrap blocked the frontend-regression audit before browser evidence could be collected.",
            overall_verdict="scan-blocked",
        )
    if skill_id == "python-ts-security-posture-audit":
        return simple_findings_blocked_summary(
            skill_id,
            repo,
            state,
