...
OUT_DIR="$HARNESS_DIR/skills/ts-frontend-regression-audit"
...
pooh_runtime_update "running" "" "Running TypeScript frontend regression audit."

python3 "$SCRIPT_DIR/run_ts_frontend_regression_scan.py" \
  --repo "$REPO_ROOT" \
...
python3 "$SCRIPT_DIR/validate_ts_frontend_regression_summary.py" \
  --summary "$SUMMARY_PATH"

pooh_runtime_finalize

echo "TypeScript frontend regression audit complete."
